...
#%% Função Objetivo (sem penalizações nem custo de degradação)
def objective(x):
...
#%% Limites
n_vars = 2 * N_batteries * T + T
lb = np.zeros(n_vars)
ub = np.ones(n_vars)

#%% Otimização com PSO
x_opt, fopt = pso(objective, lb, ub, swarmsize=40, maxiter=50, debug=True)
...
results_df.to_csv(os.path.join(output_folder, "pso_results.csv"), index=False)
...
print(f"Custo Total (PSO): {results_df['Cost'].sum():.2f} €")
